...
import com.b2international.snowowl.snomed.core.store.query.Select.Multiple;
import com.b2international.snowowl.snomed.core.store.query.Select.NestedObject;
...
public class ElasticsearchSearchRequestBuilder {
	...
	public ElasticsearchSearchRequestBuilder(ElasticsearchQueryBuilder queryBuilder) {
	...
	public SearchRequest build(Query query) {
		...
		SearchRequest searchRequest = new SearchRequest().source(searchSourceBuilder).indices(SnomedIndexConstants.INDEX_NAME);
		if (ComponentType.CONCEPT.equals(selectType)) {
			searchRequest.types(SnomedIndexConstants.CONCEPT_OBJECT_TYPE);
		} else {
			// TODO
		}
